...
from data.load import TensorDataset, PDEloader
...
def training_loop(
    ...
    for pde in dataset_kwargs.pde_name.split("-"):
        print(f">>> Initializing Dataset: {pde} <<<")
        pde_loader = PDEloader(pde)
        dataset, label = pde_loader.load_data(dataset_kwargs.path)
    ...
    dataset_obj = TensorDataset(data_train, label)

    dataset_kwargs.max_size = dataset_obj.max_size
