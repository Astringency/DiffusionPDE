...
def get_reaction_diffusion_loss(a, u, a_GT, u_GT, a_mask, u_mask, device=torch.device('cuda')):
...
def generate_reaction_diffusion(config):
    ...
    for i, (sigma_t_cur, sigma_t_next) in tqdm.tqdm(list(enumerate(zip(sigma_t_steps[:-1], sigma_t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        # Compute the loss
        pde_loss, observation_loss_a, observation_loss_u = get_ns_nonbounded_loss(a_N, u_N, a_GT, u_GT, known_index_a, known_index_u, net.img_resolution, device=device)
    ...
    if config['output']['save']:
        scipy.io.savemat(f'{config['output']['file_path']}/ns_nonbounded_results.mat', {'a': a_final, 'u': u_final})
        print(f'Done. Results saved to {config['output']['file_path']}')
