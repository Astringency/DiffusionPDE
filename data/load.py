...
from data.specs import get_pde_spec, normalize_pde_name
...
class PDEloader:
    def __init__(self, pde: str):
        self.pde = normalize_pde_name(pde)
    ...
    def _pde_dir(self, data_path):
        ...
        elif self.pde == "shallow_water":
            candidates.append(path / "shallow-water")
        ...
        for file_path in file_paths:
            with h5py.File(file_path, "r") as f:
                ...
                for local_idx, k in enumerate(tqdm(sample_keys)):
                    ...
                    init_idx = 50 if selected_format == "diffusionpde_old_rd" and arr.shape[0] > 50 else 0
                    u0 = np.expand_dims(arr[init_idx, :, :, 0], axis=0)
                    v0 = np.expand_dims(arr[init_idx, :, :, 1], axis=0)
    ...
    def _reaction_diffusion_paths(
        ...
        for pde_dir in pde_dirs:
            old_diffusion_path = pde_dir / "2D_diff-react_NA_NA.h5"
            if old_diffusion_path.exists():
                return [old_diffusion_path], "diffusionpde_old_rd", candidate_formats, sorted(detected_init_modes), mixed_init_modes
        return [], "", candidate_formats, sorted(detected_init_modes), mixed_init_modes

    @staticmethod
    def _reaction_diffusion_file_format(path):
        name = path.name
        if name == "2D_diff-react_NA_NA.h5":
            return "diffusionpde_old_rd"
